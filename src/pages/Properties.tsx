      ...
      <div className="container mx-auto px-6 lg:px-8">
        ...
        {data.properties.length === 0 ? (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="bg-white p-16 rounded-[2.5rem] border border-gray-100 shadow-sm text-center max-w-3xl mx-auto"
          >
            <Home className="w-16 h-16 text-brand-secondary/30 mx-auto mb-6" />
            <h3 className="font-heading font-bold text-2xl text-brand-primary mb-4">No Properties Available</h3>
            <p className="text-brand-muted text-lg font-light mb-8">We are currently curating new premium listings. Please check back later or contact an advisor for upcoming allocations.</p>
            <a 
              href={data.contact.whatsappUrl}
              target="_blank" 
              rel="noopener noreferrer" 
              className="inline-flex items-center justify-center bg-brand-primary text-white hover:bg-brand-secondary font-semibold py-4 px-10 rounded-xl transition-all duration-300"
            >
              Contact an Advisor
            </a>
          </motion.div>
